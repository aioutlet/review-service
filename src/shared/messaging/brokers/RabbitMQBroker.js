...
class RabbitMQBroker extends IMessageBroker {
  constructor(rabbitmqUrl, queueConfig) {
    super();
    this.rabbitmqUrl = rabbitmqUrl;
    this.queueConfig = queueConfig || config.messaging;
  ...
  async setupExchangesAndQueues() {
    try {
      const { exchanges, queues } = this.queueConfig;
   ...
   * @param {Function} handler - Function to process the event
   */
      ...
      await this.channel.consume(
        ...
        async (msg) => {
          ...
          try {
            ...
            const eventHandler = this.eventHandlers.get(content.eventType);
            if (eventHandler) {
              await eventHandler(content, correlationId);
            } else {
              // Use default handler
